...
class Dispatcher(PlugIn):
    ...
    def streamErrorHandler(self, conn, error):
        ...
        if name in stream_exceptions: 
            exc = stream_exceptions[name]
        else: 
            exc = StreamError
        
        if not exc == SystemShutdown:
            raise exc((name, text))
            ...
            if isinstance(session._expected[ID], tuple):
                ...
                try: 
                    ...
                    if typ.__class__.__name__ != 'NodeProcessed':
                        self.DEBUG("An error has occured while dispatching stanzas: %s" % (typ), 'error')
                        raise
            else:
    ...
    def disconnect(self):
        """ Send a stream terminator and handle all incoming stanzas before stream closure."""
