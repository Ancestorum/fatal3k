...
class Roster(PlugIn):
    ...
    def RosterIqHandler(self,dis,stanza):
        ...
        uself = '%s@%s' % (self._owner.User,self._owner.Server)
        self._data[uself]={'resources':{},'name':None,'ask':None,'subscription':None,'groups':None,}
        self.set=1
    ...
    def PresenceHandler(self, dis, pres):
        ...
        jstrp = jid.getStripped()
        
        if not jstrp in self._data: 
            self._data[jstrp] = {'name': None, 'ask': None, 'subscription': 'none', 'groups': ['Not in roster'], 'resources': {}}
        
        item = self._data[jstrp]
        typ = pres.getType()
        rsrc = jid.getResource()

        if not typ:
            rjid = pres.getJid()
            
            if not rjid:
                rjid = str(pres.getFrom())
            
            aff = pres.getAffiliation()
            role = pres.getRole()
        ...
        elif typ == 'unavailable' and rsrc in item['resources']:
            del item['resources'][rsrc]
        elif typ == 'error':
            pass
