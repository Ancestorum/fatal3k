...
def getPrivacyLists(disp):
    ...
    try:
        ...
        for lst in resp.getQueryPayload():
            if lst.getName() == 'list': 
                lstdc['lists'].append(lst.getAttr('name'))
            else: 
                lstdc[lst.getName()] = lst.getAttr('name')
...
def setPrivacyList(disp, prlst):
    ...
    try:
        iq = Iq('set')
        query = iq.NT.query
        query['xmlns'] = NS_PRIVACY
        query.addChild(node=prlst)
