	...
	if text == PSTO_ERROR:
		...
		try:
			...
			if psto_comm:
				...
				new_text = '\n'.join(new_text.split('\n')[:-1])
				if len(new_text) > psto_msg_trunc: new_text = new_text[:psto_msg_trunc] + '…'
				...
				for tmp in range(int(psto_comm[0]),int(psto_comm[1])+1):
					try: msg += '• #%s/%s %s' % (psto_idn,tmp,text.split(splitter)[1].split('#%s/%s' % (psto_idn,tmp))[1].split('#%s/%s' % (psto_idn,tmp+1))[0].replace('\n',' ',1))
					except: pass
				new_text += '\n' + msg
