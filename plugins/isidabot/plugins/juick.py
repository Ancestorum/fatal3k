...
def juick_user(type, jid, nick, text):
	...
	for t in result[:GT('juick_user_post_limit')]:
		tm = '#%s' % t['mid']
		if 'tags' in t: tm = '%s *%s' % (tm, ' *'.join(t['tags']))
		tm = '%s | %s' % (tm,replacer(t['body'][:GT('juick_user_post_size')].replace('\n',' \\ ')+['','…'][len(t['body']) > GT('juick_user_post_size')]))
		if 'replies' in t: tm = '%s %s' % (tm, L('Replies: %s') % t['replies'])
		tp.append(tm)
	msg = L('Found: %s') % '%s\n%s' % (text,'\n'.join(tp))
...
def juick_msg(type, jid, nick, text):
	...
	if 'video' in result:
		media = result['video']
		media = L('Attach: %s') % media[list(media.keys())[0]]
	elif 'photo' in result: media = L('Attach: %s') % result['photo']['medium']
	else: media = ''
