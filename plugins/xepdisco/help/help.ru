disco.ccat = *, all, info, muc, xepdisco
disco.desc = Показывает результаты обзора сервисов для указанного JID.\nТакже можно запросить обзор по узлу (node). Формат запроса jid#node.\nВторой или третий (если также задается ограничитель кол-ва) параметр - поиск.\nИщет заданное слово в JID и описании элемента диско.\nЕсли поисковым словом задать имя конференции до названия сервера (например qwerty@),\nто покажет место этой конференции в общем рейтинге.\nВ общий чат может дать max 50 результатов, без указания кол-ва - 10.\nВ приват может дать max 250, без указания кол-ва 50.
